                    ...
                    <form v-on:submit="guardar">
                        <div class="input-group mb-3">
                            ...
                            <select v-model="libro_id" id="libro_id" required class="form-control">
                                <option v-for="libro in libros" :key="libro.id" :value="libro.id">{{ libro.id }} {{ libro.titulo }}</option>
                            </select>
                        </div>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <select v-model="cliente_id" id="cliente_id" required class="form-control">
                                <option v-for="cliente in clientes" :key="cliente.id" :value="cliente.id">{{ cliente.id }} {{ cliente.name }}</option>
                            </select>
                        </div>

                        <div class="input-group mb-3">
                            <span class="input-group-text"><i class="fa-solid fa-calendar-days"></i></span>
                            <input type="date" v-model="fecha_prestamo" id="fecha_prestamo" placeholder="Seleccione la fecha de prestamo" required class="form-control">
                        </div>
                        ...
                        <div class="input-group mb-3">
                            ...
                            <!-- <select v-model="estado" id="estado" required class="form-control">
                                <option value="En prestamo" :selected="selected">En prestamo</option>
                            </select> -->
        ...
        methods:{
            guardar(){
                ...
                if (this.libro_id === '') {
                    ...
                    enviarSolicitud('POST', parametros, this.url, 'Prestamos registrado')
                }
            },
            getLibros(){
